...
public class MainVerticle extends AbstractVerticle {
	...
	Future<Router> configureRouter(Void unused) {
		...
		router.get("/photos") // auch: /photos?photoTitle=Bild1?tag=tagName; /photos/:albumID
			.handler(authenticationHandler::isLoggedIn)
			.handler(ctx -> {
		...
		router.delete("/albums/photo")
			...
			.handler(photoHandler::photoIsUser)
			.handler(albumHandler::albumIsUser)
			.handler(albumHandler::validatePhotoIsInAlbum)
			.handler(albumHandler::removePhotoFromAlbum);
		...
		router.get("/albums/contains/:photoID")
			.handler(authenticationHandler::isLoggedIn)
			.handler(ctx -> {
				ctx.data().put("photoID", ctx.pathParam("photoID"));
				ctx.next();
